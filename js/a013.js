...
addEventListener('load', function () {
	...
	game.onload = function () {
		...
		// ノーツのサーフェスを定義
		var surface = new Surface(75, 35);
		// 色付き四角形を描く
		surface.context.fillStyle = "#B1CFFC";
		surface.context.fillRect(0, 0, 74, 35);

		var notes = [];

		for(var i = 0; i < 6; i++){
			// ノーツを生成
			notes[i] = new Sprite(75, 35);
			// Surfaceオブジェクトを生成しスプライトに連結
			notes[i].image = surface;

			notes[i].x = 95 + (75 * i) + 1;
			notes[i].y = 0 + (70 * i);
			game.rootScene.addChild(notes[i]);

			notes[i].addEventListener("enterframe", function(){
				// this.y += 10 / 4;
				this.y += 10 / 2;
				// this.y += 10;
				// this.y += 10 * 2;
